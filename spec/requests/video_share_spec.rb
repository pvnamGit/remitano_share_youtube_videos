  ...
  describe "get my shared videos" do
    ...
    context "when user is not authenticated" do
      it "redirects to sign up page" do
        expect redirect_to('/user/new')
      end
    end
  end

  describe "delete videos" do
    let(:video) { VideoShare.create(title: "Test Title", description: "Test Description 1", thumbnail_url: "http://example.com/thumbnail1", user_id: @user.id) }
    before do
      delete "/my_videos/#{video.id}"
    end
